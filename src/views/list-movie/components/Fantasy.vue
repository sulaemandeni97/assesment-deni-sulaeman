        ...
        >
          <img
            class="
              ...
              md:rounded-[40px]
            "
            :src="'https://image.tmdb.org/t/p/original/' + fantasy.poster_path"
            @click="onShowDetail(fantasy.id)"
          />
        </div>
      </div>
    </div>
    ...
    const onFetchFantasy = () => {
      return new Promise((resolve, reject) => {
        axios
          ...
          .then((response) => (fantasies.value = response.data.results))

        ...
        >
          <img
            class="
              ...
              md:rounded-[40px]
            "
            :src="'https://image.tmdb.org/t/p/original/' + comedy.poster_path"
            @click="onShowDetail(comedy.id)"
          />
        </div>
      </div>
    </div>
    ...
    const onFetchComedy = () => {
      return new Promise((resolve, reject) => {
        axios
          ...
          .then((response) => (comedies.value = response.data.results))
